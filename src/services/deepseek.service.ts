...
export class DeepSeekService {
  ...
  static async analyzeResume(name: string, jobTitle: string, skills: string[], answers?: Record<string, string>, resumeText?: string, knockoutSkills: string[] = []) {
    const systemPrompt = "You are DeepSeek-R1, an expert AI recruitment evaluator. Respond strictly in JSON.";
    const prompt = `
  ...
  static async generateJobBaseline(title: string, department?: string) {
    const systemPrompt = "You are DeepSeek-R1, an expert AI recruitment strategist. Respond strictly in JSON.";
    const prompt = `
  ...
  static async generateDashboardInsight(orgContext: any) {
    const systemPrompt = "You are DeepSeek-R1, an expert AI recruitment advisor. Respond with a single sentence.";
    const prompt = `Context: ${JSON.stringify(orgContext)}. Give one actionable tip.`;
    
    if (!config.deepseekApiKey) return "Add DeepSeek API key for more insights.";
    
    const res = await this.callApi(prompt, systemPrompt);
    return res.insight || res; // Handle both JSON or string return
  }
}
